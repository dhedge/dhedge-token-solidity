...
contract("DHedgeTokenTest", (accounts) => {
  beforeEach(async () => {
    ...
    proxy = await DHedgeTokenProxy.new(delegate.address, initData, {
      from: proxyAdmin,
    });
  ...
  it("First version of token is initialized correctly", async () => {
    assert(
      "dHedge DAO Token" === (await token.name.call({ from: user1 })),
      "name doesn't match"
    );
    assert(
      "DHT" === (await token.symbol.call({ from: user1 })),
      "symbol doesn't match"
    );
    assert(
      1 === (await token.version.call({ from: user1 })).toNumber(),
      "version doesn't match"
    );
    assert(
      totalSupply.isEqualTo(
        await token.balanceOf.call(tokenOwner, { from: user1 })
      ),
      "token owner doesn't have all supply"
    );

    assert(
      delegate.address ===
        (await proxy.implementation.call({ from: proxyAdmin })),
      "delegate address doesn't match"
    );

    try {
      await token.__DHedgeTokenV1_init(user1, { from: user1 });
      assert(false);
    } catch (e) {
  ...
  it("Token upgrade 2in1 works correctly", async () => {
    ...
    let initData = web3.eth.abi.encodeFunctionCall(
      {
        inputs: [],
        name: "__DHedgeTokenV2Example_init",
        outputs: [],
        stateMutability: "nonpayable",
        type: "function",
      },
    ...
    await proxy.upgradeToAndCall(delegate2.address, initData, {
      from: proxyAdmin,
    });
    ...
    assert(
      2 === (await token.version.call({ from: user1 })).toNumber(),
      "version doesn't match"
    );

    //check new function works
    assert(
      "Hello" === (await token.brandNewFunction.call({ from: user1 })),
    ...
    assert(
      new BigNumber(1).isEqualTo(await web3.eth.getBalance(token.address)),
      "ether balance matches"
    );
    assert(
      new BigNumber(1).isEqualTo(
        await token.receivedEther.call({ from: user1 })
      ),
      "receivedEther value matches"
    );
  });
  ...
  it("Token upgrade 1by1 works correctly", async () => {
    ...
    await token.__DHedgeTokenV2Example_init({ from: user1 });
  ...
  it("Can transfer tokens to any address", async () => {
    ...
    assert(
      oneToken.plus(tokenBalanceUserBefore).isEqualTo(tokenBalanceUserAfter),
      "user token balance doesn't match"
    );
  });
